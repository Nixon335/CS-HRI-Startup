...
public class FF2DGrid{
  ...
  // added variables to progressGen function, to pass through fuel, refractory period, and wind direction
  public void progressGen(int[][] fuel, int[][] refract, int biasX, int biasY){ 

    int i; int j;                                         // local iterators
    // Array of booleans to represent on fire or dormant 
    boolean[][] isNextCellOnFire=new boolean[size2D[0]][size2D[1]];  // stores a local next state array
    Random rand = new Random();

    for(i=0;i<size2D[0];i++) {                            // for each cell array position
      for(j=0;j<size2D[1];j++) { 
    	  
    	// get the fuel count in the specific cell
    	int cellFuel = fuel[i][j];
    	// get the fire status of the current cell
    	boolean onFire = cells[i][j].getState();
    	// get whether the cell will be on fire in the next iteration
    	isNextCellOnFire[i][j]=cells[i][j].nextState(cellFuel, onFire, biasX, biasY);              // copy next state to the store array
    	
    	// if so, decrease fuel by 1
        if(isNextCellOnFire[i][j]){
        	fuel[i][j] -= 1 ;
        }
        // else if the fuel is 0, its either just finished its refractory period, or is still in it
        else if (fuel[i][j]==0) {
        	// If its refract count = 0 then refresh the fuel and refract values
            if (refract[i][j]==0){
            	refract[i][j]=rand.nextInt(6)+20;		// For random refractory periods
            	fuel[i][j]=rand.nextInt(6)+15;			// For when using random fuel values
            	//refract[i][j] = 10;						// For fixed refractory periods
            	//fuel[i][j] = 15;						// For when using fixed fuel values
            }
            // Else decrease refract by 1 
            else {
            	refract[i][j] -=1;
            }
        }
    ...
    for(i=0;i<size2D[0];i++) {                           // for each cell array position
      for(j=0;j<size2D[1];j++) { 
        cells[i][j].setState(1,1,isNextCellOnFire[i][j]);           // Use the store array to set all next states
      }
    }
