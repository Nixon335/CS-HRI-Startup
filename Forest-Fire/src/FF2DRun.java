...
public class FF2DRun extends JPanel{        // The code must extend Jpanel in the declaration
  ...
  private static int numberOnFire(boolean[][] grid) { 
	  int number = 0;
  ...
  public static void main(String[] argv) {   // Standard main (with command line input)
    ...
    frame.setTitle("Team 7's FOREST FIRE SIMULATOR 2K16");       		 // set Title of Frame
    ...
    try {                                           // try-catch Exception handling to open an output textfile
      ...
      // Populate each cell with fuel
      int[][] fuel = new int[xSizeIn][ySizeIn];
      for (i=0; i<xSizeIn; i++){
    	  for (j=0; j<ySizeIn; j++){
    		  fuel[i][j] = rand.nextInt(6)+15;	// Version for implementing random fuel distribution 
    		  //fuel[i][j] = 15;				// Version for static fuel 
    	  }
      }
      
      // Mark each cell with a refractory time in iterations
      int[][] refract = new int[xSizeIn][ySizeIn];
      for (i=0; i<xSizeIn; i++){
    	  for (j=0; j<ySizeIn; j++){
    		  refract[i][j] = rand.nextInt(6)+15;	// Implementation of random refractory period
    		  //refract[i][j] = 10;					// Static refractory period 
    	  }
      }
      
      
      int[] fireArray = new int[gensIn+1];
      
      for(i=0;i<=gensIn;i++){                       // iterate through generations
        ...
        //System.out.println("Iteration " + i + " cells on fire: " + numberOnFire(store));
        System.out.println(numberOnFire(store));
        fireArray[i] = numberOnFire(store);
        ...
        // Set coordinates for wind direction (between 0 and 2)
        int windX = 0;
        int windY = 0;
        		
        // Generate the next states for the following iteration
        if(i!=gensIn){ cellSet.progressGen(fuel, refract, windX, windY); }						// the next states
        
        
      }
      
      System.out.println("List of FIRE ");
      for (i=0; i<=gensIn; i++){
    	  System.out.print(fireArray[i] + " ");
      }
