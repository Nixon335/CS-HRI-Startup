...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        if (LambdaProcedure, MuProcedure):
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        if not scheme_symbolp(clause.first):
            raise SchemeError
        formals.append(clause.first)
        vals.append(scheme_eval(clause.second.first, env))
    ...
    return env.make_child_frame(list_to_pair(formals), list_to_pair(vals))
    # END PROBLEM 15
